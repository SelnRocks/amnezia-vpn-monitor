...
<head>
    <meta charset="UTF-8">
    <title>Панель AmneziaVPN — Мониторинг</title>
    <style>
        ...
        async function updateData() {
            try {
                ...
                clients.forEach((client, idx) => {
                    ...
                    // Поменяли местами ячейки final_sent и final_received в верстке скрипта
                    row.innerHTML = `
                        <td>${idx + 1}</td>
                        <td>${badge}</td>
...
<body>
    <div class="container">
        <h1>Мониторинг клиентов AmneziaVPN</h1>

        <table>
            <thead id="vpnHeader">
                <tr>
                    <th style="width: 3%">#</th>
                    <th style="width: 8%" data-field="protocol" onclick="handleHeaderClick('protocol')">Протокол<span class="sort-indicator"> ↕</span></th>
                    <th style="width: 20%" data-field="name" onclick="handleHeaderClick('name')">Имя клиента<span class="sort-indicator"> ↕</span></th>
                    <th style="width: 11%" data-field="ip" onclick="handleHeaderClick('ip')">Внутренний IP<span class="sort-indicator"> ↕</span></th>
                    <th style="width: 24%" data-field="handshake" onclick="handleHeaderClick('handshake')">Последнее рукопожатие<span class="sort-indicator"> ↕</span></th>
                    <!-- Поменяли местами заголовки "Отдано" и "Скачано" -->
                    <th style="width: 12%" data-field="sent" onclick="handleHeaderClick('sent')">Скачано<span class="sort-indicator"> ↕</span></th>
                    <th style="width: 12%" data-field="received" onclick="handleHeaderClick('received')">Отдано<span class="sort-indicator"> ↕</span></th>
                    <th style="width: 10%" data-field="date" onclick="handleHeaderClick('date')">Дата создания<span class="sort-indicator"> ▲</span></th>
                </tr>
            </thead>
            <tbody id="vpnBody">
                ...
                <tr>
                    ...
                    <td><span class="ip">{{ client.allowed_ips }}</span></td>
                    <td><span class="handshake-time">{{ client.final_handshake }}</span></td>
                    <!-- Поменяли местами переменные Jinja2 для первой отрисовки страницы -->
                    <td><span class="traffic-val">{{ client.final_sent }}</span></td>
                    <td><span class="traffic-val">{{ client.final_received }}</span></td>
                    <td><span class="date">{{ client.creation_date }}</span></td>
                </tr>
...
def traffic_to_bytes(traffic_str):
    if not traffic_str or traffic_str in ["0 B", "-", "Нет данных"]:
        return 0
    try:
...
def handshake_to_seconds(hs_str):
    if not hs_str or any(x in hs_str for x in ["Ни разу", "Нет данных", "Never"]):
        return 9999999999
    try:
        total_seconds = 0
        hs_str = hs_str.lower()
...
# Разбор IP-адреса "10.8.1.2" на составляющие для идеальной числовой сортировки [10, 8, 1, 2]
...
def parse_wg_live(container_name):
    ...
    for peer_key, peer_body in peers:
        handshake, rcv, snt, ips = "Ни разу не подключался", "0 B", "0 B", "-"
...
def get_all_amnezia_parameters(sort_by="date", sort_dir="desc"):
    ...
    for container in containers:
            ...
            if res.returncode == 0:
                ...
                for item in json_data:
                    ...
                    if p_key:
                        ...
                        names_map[p_key] = u_data.get("clientName", "Без имени")
        ...
        for peer_key, live_info in live_wg.items():
            try:
                client_name = names_map.get(peer_key, f"Неизвестный ({peer_key[:8]})")
                ...
                all_clients.append({
                    ...
                    # Внутренние ключи сортировки на бэкенде
    ...
    # 🔥 МОЩНАЯ СЕРВЕРНАЯ СОРТИРОВКА PYTHON: Исключает любые сбои в браузере!
...
def index():
    try:
        # Первая загрузка: сортируем по умолчанию по дате создания (desc)
        clients = get_all_amnezia_parameters(sort_by="date", sort_dir="desc")
        from jinja2 import Template
        tmpl = Template(HTML_TEMPLATE)
        return tmpl.render(clients=clients)
    except Exception as e:
        return f"<pre>Ошибка сервера:\n{traceback.format_exc()}</pre>", 500

# Динамический API-роут теперь принимает параметры сортировки из браузера
